...
class Lock {
  public:
    ...
    char* getName() { return name; }	// debugging assist
    char* getOwnerName() {return lockOwner->getName();} //debugging assist
  ...
  private:
    ...
    bool isFree;                        // a boolean checking if the 
                                        // lock is FREE or BUSY
...
class Condition {
  ...
  private:
    ...
    Lock* waitingLock;                  // a lock to make sure only one 
                                        // thread is using the condition
                                        // variable at a time
};
#endif // SYNCH_H
